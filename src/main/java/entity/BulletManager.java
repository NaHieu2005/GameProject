...
public class BulletManager implements Runnable{
    ...
    public BulletManager(double x, double y) {
        this.x = x;
        this.y = y;
        start();
    }

    public ArrayList<BulletSpawner> getBlueButterfly(){
        ArrayList<BulletSpawner> b = new ArrayList<>();
        for (int i = 0; i < blue_butterfly.size(); i++) {
            b.add(new BulletSpawner(blue_butterfly.get(i), blue_butterfly.get(i).speed));
        }
        b.addAll(blue_butterfly);
        return b;
    }

    public ArrayList<BulletSpawner> getPurpleButterfly(){
        ArrayList<BulletSpawner> b = new ArrayList<>();
        for (int i = 0; i < purple_butterfly.size(); i++) {
            b.add(new BulletSpawner(purple_butterfly.get(i), purple_butterfly.get(i).speed));
        }
        return b;
    }

    public ArrayList<BulletSpawner> getRedButterfly1(){
        ArrayList<BulletSpawner> b = new ArrayList<>();
        for (int i = 0; i < red_butterfly_1.size(); i++) {
            b.add(new BulletSpawner(red_butterfly_1.get(i), red_butterfly_1.get(i).speed));
        }
        return b;
    }

    public ArrayList<BulletSpawner> getRedButterfly2(){
        ArrayList<BulletSpawner> b = new ArrayList<>();
        for (int i = 0; i < red_butterfly_2.size(); i++) {
            b.add(new BulletSpawner(red_butterfly_2.get(i), red_butterfly_2.get(i).speed));
        }
        return b;
    }

    public ArrayList<BulletSpawner> getBubble(){
        ArrayList<BulletSpawner> b = new ArrayList<>();
        for (int i = 0; i < bubble.size(); i++) {
            b.add(new BulletSpawner(bubble.get(i), bubble.get(i).speed));
        }
        return b;
    }
